...
constraints convert as a documented 🟡 (`CreateMockToDoubleRector`); only the unmappable links
(`willReturnMap`/`willReturnSelf`, `getMockBuilder`, `prophesize`, and `with()` constraints with no
`Argument` form) stay manual.
