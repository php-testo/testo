...
constraints convert as a documented 🟡 (`CreateMockToDoubleRector`), including intersection stubs,
`getMockBuilder(X)->disableOriginalConstructor()->getMock()`, `willReturnSelf()`, `withAnyParameters()`,
and the comparison/string/`logicalNot`/`logicalOr` constraints (via `satisfies`/`not`/`any`); only the
unmappable links (`willReturnMap`, a builder step beyond `disableOriginalConstructor`, `prophesize`,
`logicalAnd`, delta/canonicalizing equality) stay manual.
