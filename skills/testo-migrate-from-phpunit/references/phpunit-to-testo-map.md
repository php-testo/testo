...
| `$this->assertMatchesRegularExpression($p, $s)` / `assertDoesNotMatchRegularExpression` (PHPUnit 9: `assertRegExp` / `assertNotRegExp`) | `Assert::string($s)->matchesRegex($p)` / `->notMatchesRegex($p)` — subject first, same full PCRE pattern. Rector converts all four names. |
...
| `$this->expectExceptionMessageMatches('/.../')` | `->withMessageMatchingRegex('/.../')`. |
...
- **Tests inherited from a `vendor/` PHPUnit base** (e.g. a shared `AbstractCollectorTestCase`) cannot be ported in place: the base stays PHPUnit and its assertions are invisible to Testo. Copy the base into a local trait or abstract class under the test tree (e.g. `tests/Support/`), run the Rector pass over it with the rest of the scope, and point the subclasses at the copy. A base that boots an application in `setUp()` usually cleans it up in `tearDown()`: carry that over as an `#[AfterTest]` method, not only the `#[BeforeTest]` part. The Rector pass marks the class extending such a base with a `#[Skip]` naming it; remove that attribute once the class points at the ported copy.
...
- **Assertion counts differ from PHPUnit** (`Expect::exception()`, a chained `Assert::array()->hasKeys()->…` count differently). Verify the port by the test count, not the assertion count. Testo still reports it: `totals.assertions` in `--json`, and `assertions` per `<testcase>` in `--log-junit`, same as PHPUnit's JUnit.
