...
- `#[Skip]` next to `#[Retry]` / `#[Repeat]` wins even harder than that: the test is reported as skipped at the entry of its pipeline, outer to the retry/repeat loop, so the loop is not started at all (the case itself still runs, with its class-level hooks, as long as it has another test to run).
