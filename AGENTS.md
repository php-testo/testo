...
- When you deprecate public API, add its migration to the `testo-shift` Rector set ([`bridge/rector/config/testo-shift.php`](bridge/rector/config/testo-shift.php)) with fixtures in `bridge/rector/config/testo-shift/`, and list it in the shift section of `FEATURE_PARITY.md`. Users run that set to move off deprecated API before it is removed.
- Naming a new assertion chain method (`Assert::string()->…` and the other typed heads): follow [docs/spec/assert-naming.md](docs/spec/assert-naming.md).
