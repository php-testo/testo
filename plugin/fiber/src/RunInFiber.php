 ...
 *
 * Either way the test gets its own coroutine scope: {@see Coroutine::spawn()} adds coroutines to the
 * test's schedule, {@see Coroutine::await()} / {@see Coroutine::concurrently()} wait for them.
 *
 ...
 *
 * Self-wiring: it is {@see Interceptable}, so {@see RunInFiberInterceptor} (the fiber wrap) and
 * {@see CoroutineScopeInterceptor} (the coroutine scope) are inserted only for tests that carry it —
